...
class ar_settlement extends CI_Controller
{
	...
	function index()
	{
        $data = array(
            ...
            'breadcrumb' => '<li>Finance</li>
                             <li>AR Settlement</li>',
            'page_icon' => 'icon-clipboard-text',
            'page_title' => 'AR Settlement',
            'page_subtitle' => '',
            'custom_scripts' => '<script>$("#menu_ars").addClass("active");</script>'
            
            
        );
        $this->template->build('v_ar_settlement', $data);
    }
	
	function populate()
	{
		$aColumns = array('id','no_faktur','billing_no','nama','jumlah_tagihan','jumlah_pembayaran','sisa_tagihan','settlement_status','status');
        
        // DB table to use
        //$sTable = 'v_so_header';
         $sTable = '(SELECT bill.id, bill.`id_header`, head.`id_customer`,bill.settlement_status,
...
GROUP BY bill.`id` HAVING sisa_tagihan<1
        ...
        foreach ($rResult->result_array() as $aRow) {
            ...
            $row[] = '<input id="settler" type="checkbox" '.($aRow['settlement_status']=='1'?'checked disabled':'').' > AR Settlement';
	...
	{
		$select = $this->db->query("SELECT bill.id, bill.`id_header`, head.`id_customer`,
 bill.`billing_no`, head.`no_faktur`,
cust.`nama`,bill.`jumlah_tagihan`, 
SUM(bayar.jumlah_pembayaran) AS jumlah_pembayaran, 
(bill.`jumlah_tagihan`)-IF(bayar.jumlah_pembayaran IS NULL,'0',SUM(bayar.`jumlah_pembayaran`)) AS sisa_tagihan,
...
GROUP BY bill.`id`
	...
	function settleAR()
	{
		$billing_id = $this->input->get('id');
		$settle = $this->db->query("update ck_billing_statement set settlement_status='1' where id='$billing_id'");
		$getheaderid = $this->db->query("select id_header from ck_billing_statement where id='$billing_id'");
		
		$header_id; // get header id
		foreach($getheaderid->result() as $x)
			$header_id = $x->id_header;
		//set status di header	
		$updateheader = $this->db->query("update ck_po_customer_header set status='settled' where id='$header_id'");
		if($settle)
			echo "done";
		else echo "fail";
	}
