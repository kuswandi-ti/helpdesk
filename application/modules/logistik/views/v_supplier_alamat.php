...
<script>
	$(document).ready(function() {
		$('#table_data').DataTable({
			...
			"bServerSide": true,
			"sServerMethod": "GET",
			"sAjaxSource": "logistik/supplier_alamat/get_data/<?php echo $this->input->get('sid'); ?>",
			"iDisplayLength": 10,
			...
			"aoColumns": [
				...
				{ "bVisible": true, "bSearchable": true, "bSortable": true }, // PIC HP -> CP No. HP 8
		...
		$(".btn-tambah").click(function() {
			$('#modal-primary-header').html('Tambah Data Alamat Supplier');
			reset_input();
		});
		...
		$("#cboprovinsi").on('change',function(){
			...
			$.ajax({
				url: 'logistik/supplier_alamat/get_kabkota/'+id_provinsi,
				dataType: 'json',
				type: 'get',
		...
		$("body").on("click", "#table_data .btn-edit", function() {
			...
			$.ajax({
				url : 'logistik/supplier_alamat/supplier_alamat_view',
				type: 'POST',
				data: {
					id: id
				},
				...
				success : function(data) {
					...
					$('#modal-primary-header').html('Edit Data Alamat Supplier');
					$('#txtid').val(data.id);
					$('#cbosupplier').val(data.id_supplier);
		...
		$(".add-edit").click(function(e) {			
			...
			if ($('#txtid').val() == '') {
				var urls = 'logistik/supplier_alamat/supplier_alamat_create';
			} else {
				var urls = 'logistik/supplier_alamat/supplier_alamat_update';
			}
