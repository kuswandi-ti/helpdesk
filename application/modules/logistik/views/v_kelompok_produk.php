...
<script>
	$(document).ready(function() {
		$('#table_data').DataTable({
			...
			"bServerSide": true,
			"sServerMethod": "GET",
			"sAjaxSource": "logistik/kelompok_produk/get_data",
			"iDisplayLength": 10,
			...
			"aoColumns": [
				...
				{ "bVisible": true, "bSearchable": true, "bSortable": true }, // Nama Perbekalan 4
		...
		$("body").on("click", "#table_data .btn-edit", function() {
			...
			$.ajax({
				url : 'logistik/kelompok_produk/kelompok_produk_view',
				type: 'POST',
				data: {
					id: id
				},
				...
				success : function(data) {
					...
					$('#txtnama_edit').val(data.nama_kelompok);
					$('#txtdeskripsi_edit').val(data.deskripsi);
					$('#cboprodukperbekalan_edit').selectpicker('val', data.produk_perbekalan_id);
				},
				error: function() {
				  alert("failure");
				}
				...
				<table id="table_data" class="table table-head-custom table-striped" style="width: 100%">
					<thead>
						<tr>
							...
							<th>Nama</th>
							<th>Deskripsi</th>
							<th class="text-center">Perbekalan Farmasi</th>
...
<div class="modal fade" id="modal-create" tabindex="-1" role="dialog" aria-labelledby="modal-primary-header">                        
	<div class="modal-dialog modal-primary" role="document">
		...
		<form id="form-create" method="POST" action="logistik/kelompok_produk/kelompok_produk_create">
			<div class="modal-content">
				<div class="modal-header">                        
					<h4 class="modal-title" id="modal-primary-header">Tambah Data Kelompok Produk</h4>
				</div>
				<div class="modal-body">
					<div class="row">
						<div class="col-md-12">
							<label>Kode</label>
							<input type="text" name="txtkode_add" id="txtkode_add" class="form-control" placeholder="Kode Kelompok Produk" required>
						</div>
					</div>
					&nbsp;
					<div class="row">
						<div class="col-md-12">
							<label>Nama</label>
							<input type="text" name="txtnama_add" id="txtnama_add" class="form-control" placeholder="Nama Kelompok Produk" required>
						</div>
					</div>
					&nbsp;
					<div class="row">
						<div class="col-md-12">
							<label>Perbekalan Farmasi</label>
							<select name='cboprodukperbekalan_add' id='cboprodukperbekalan_add' class='form-control bs-select' data-live-search='true'>
								<?php
									if ($get_perbekalan_farmasi->num_rows() > 0) {
										foreach ($get_perbekalan_farmasi->result() as $row) {
...
<div class="modal fade" id="modal-edit" tabindex="-1" role="dialog" aria-labelledby="modal-primary-header">                        
	<div class="modal-dialog modal-primary" role="document">
		<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true" class="icon-cross"></span></button>
		<form id="form-edit" method="POST" action="logistik/kelompok_produk/kelompok_produk_update">
			<div class="modal-content">
				<div class="modal-header">                        
					<h4 class="modal-title" id="modal-primary-header">Ubah Data Kelompok Produk</h4>
				</div>
				<div class="modal-body">
					<div class="row">
						<div class="col-md-12">
							...
							<input type="text" name="txtkode_edit" id="txtkode_edit" class="form-control" placeholder="Kode Kelompok Produk" required>
						</div>
						<div class="col-md-12">
							<label>Nama</label>
							<input type="text" name="txtnama_edit" id="txtnama_edit" class="form-control" placeholder="Nama Kelompok Produk" required>
						</div>
					</div>
					&nbsp;
					<div class="row">
						<div class="col-md-12">
							<label>Perbekalan Farmasi</label>
							<select name='cboprodukperbekalan_edit' id='cboprodukperbekalan_edit' class='form-control bs-select' data-live-search='true'>
								<?php
									if ($get_perbekalan_farmasi->num_rows() > 0) {
										foreach ($get_perbekalan_farmasi->result() as $row) {
											echo "<option value='".$row->id."'>".$row->nama."</option>";
