...
<script>
	$(document).ready(function() {
		table = $('#table_data').DataTable({
					...
					"ajax": {
						"url": "logistik/produk_expired/ajax_list",
						"type": "POST",
						"data": function ( data ) {
					...
					"aoColumns": [
						...
						{ "bVisible": true, "bSearchable": true, "bSortable": true }, // Supplier 2
						{ "bVisible": true, "bSearchable": true, "bSortable": true }, // Kemasan 3
						{ "bVisible": true, "bSearchable": true, "bSortable": true }, // Satuan 4
						{ "bVisible": true, "bSearchable": true, "bSortable": true }, // Batch Number 5
						{ "bVisible": true, "bSearchable": true, "bSortable": true }, // Expired Date 6
						{ "bVisible": true, "bSearchable": true, "bSortable": true }, // Bulan / Hari 7
						{ "bVisible": true, "bSearchable": true, "bSortable": true }, // Stok 8
	...
	$("body").on("click", "#table_data .btn-detail", function() {
		...
		var vbatch_number = parentTr.find("td:eq(5)").text();
		var vexpired_date = parentTr.find("td:eq(6)").text();
		
		$.ajax({
			url: 'logistik/produk_expired/tampil_stok_produk',
			type: 'post',
			async: false,
			data: {
				...
				<table id="table_data" class="table table-head-custom table-striped" style="width: 100%">
					<thead>
						<tr>
							...
							<th class="text-center">Bulan / Hari</th>
