...
class produk_expired extends CI_Controller {
	
    public function __construct() {
		parent::__construct();
		$this->load->model('m_produk_expired');
	...
	public function index() {
		$data = array(
			'title' => 'Monitoring Produk Expired',
			'breadcrumb_home_active' => '',
            'breadcrumb' => '<li>Logistik</li>
							 <li>Inquiry</li>
							 <li>Monitoring Stok</li>
							 <li><a href="logistik/produk_expired">Monitoring Produk Expired</a></li>',
			'page_icon' => 'icon-files',
			'page_title' => 'Monitoring Produk Expired',
			'page_subtitle' => 'Monitoring Produk Expired',
			'get_supplier' => $this->m_produk_expired->get_supplier(),
			'custom_scripts' => "<script type='text/javascript'>$('#menu_logistik_produkexpired').addClass('active')</script>"
		);
		$this->template->build('v_produk_expired', $data);
	}
	
	public function ajax_list() {
        $list = $this->m_produk_expired->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        foreach ($list as $stocks) {
			...
			$row[] = $stocks->selisih_bulan.' / '.$stocks->selisih_hari;
            $row[] = $stocks->stok;			
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->m_produk_expired->count_all(),
                        "recordsFiltered" => $this->m_produk_expired->count_filtered(),
                        "data" => $data,
                );
	...
	function tampil_stok_produk() {		
        ...
        $data = $this->m_produk_expired->get_stok_produk($id_produk, $batch_number, $expired_date);
	...
	function detail_stok($id_produk,
						 $batch_number,
						 $expired_date) {
        $data = $this->m_produk_expired->detail_stok($id_produk,
												     $batch_number,
												     $expired_date);
        if ($data->num_rows() > 0) {
            $i = 1;
