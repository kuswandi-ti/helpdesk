...
class stok_produk extends CI_Controller {
	...
	public function index() {
		$data = array(
            ...
            'breadcrumb' => '<li>Logistik</li>
							 ...
							 <li><a href="logistik/stok_produk">Stok Produk</a></li>',
			'page_icon' => 'icon-3d-rotate',
			'page_title' => 'Stok Produk',
			'page_subtitle' => 'Posisi & Rincian Stok Produk',
			'custom_scripts' => "<script type='text/javascript'>$('#menu_logistik_stokproduk').addClass('active')</script>"
		);
		$this->template->build('v_stok_produk', $data);
	}
	...
	public function get_data() {
        ...
        $rResult = $this->db->get_where($sTable, array('stok >' => 0));
	...
	function tampil_stok_produk() {		
        ...
        $data = $this->m_stok_produk->get_stok_produk($id_produk);
	...
	function detail_stok($id_produk,
						 $batch_number,
						 $expired_date) {
        $data = $this->m_stok_produk->detail_stok($id_produk,
												  $batch_number,
												  $expired_date);
        if ($data->num_rows() > 0) {
            $i = 1;
