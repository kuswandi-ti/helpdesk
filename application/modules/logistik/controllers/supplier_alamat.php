...
class supplier_alamat extends CI_Controller {
	
	var $view_alamat_supplier	= 'ck_supplier_alamat_view';

	public function __construct() {
		parent::__construct();
		$this->load->model('m_supplier_alamat');
	...
	public function index() {
		$data = array(
            ...
            'breadcrumb' => '<li>Logistik</li>
			                 ...
			                 <li>Mitra Usaha</li>
                             <li><a href="logistik/supplier_alamat">Alamat Supplier</a></li>',
			'page_icon' => 'icon-road-sign',
			'page_title' => 'Alamat Supplier',
			'page_subtitle' => 'Data Master Alamat Supplier',
			'get_supplier' => $this->m_supplier_alamat->get_supplier(),
			'get_propinsi' => $this->m_supplier_alamat->get_propinsi(),
			'custom_scripts' => "<script type='text/javascript'>$('#menu_logistik_supplier_alamat').addClass('active');</script>"
		);
		$this->template->build('v_supplier_alamat', $data);
	}
	...
	public function get_data($id_supplier = '') {
         ...
         */
        $aColumns = array('id', 'id_supplier', 'nama_supplier', 'nama_alamat', 'alamat', 
		...
		$sTable = "(SELECT
				    ...
				    WHERE
						id_supplier = '".$id_supplier."') supplier_alamat";
	...
	function get_kabkota($id) {
		$this->m_supplier_alamat->get_kabkota($id);
	}
	
  	function supplier_alamat_view() {
		$id = $this->input->post('id');
		$supplier = $this->m_supplier_alamat->supplier_alamat_view($id);
	...
	function supplier_alamat_create() {
		$notifikasi = $this->m_supplier_alamat->supplier_alamat_create();
		if ($notifikasi == 1) {
	...
	function supplier_alamat_update() {
		$notifikasi = $this->m_supplier_alamat->supplier_alamat_update();
		if ($notifikasi == 1) {
			$notifikasi=" Selamat ! Anda berhasil";
		} else {
			$notifikasi=" Maaf ! Anda gagal";
	...
	function supplier_alamat_delete($id = '') {
		$notifikasi = $this->m_supplier_alamat->supplier_alamat_delete($id);
		if ($notifikasi == 1) {
			$notifikasi=" Selamat ! Anda berhasil";
		} else {
			$notifikasi=" Maaf ! Anda gagal";
