...
class forecast_pembelian extends CI_Controller {
	...
	public function __construct() {
		parent::__construct();
		$this->load->model('m_forecast_pembelian');
	...
	public function index() {
		$data = array(
			'title' => 'Forecast Pembelian Produk',
			'breadcrumb_home_active' => '',
            'breadcrumb' => '<li>Logistik</li>
			                 <li>Manajemen Data</li>
			                 <li>Perencanaan Stok</li>
                             <li><a href="logistik/forecast_pembelian">Forecast Pembelian Produk</a></li>',
			'page_icon' => 'fa fa-signal',
			'page_title' => 'Forecast Pembelian Produk',
			'page_subtitle' => 'Forecast pembelian produk ke supplier',
			'custom_scripts' => "<script type='text/javascript'>$('#menu_logistik_forecastpembelian').addClass('active');</script>"
		);
		$this->template->build('v_forecast_pembelian', $data);
	}
	...
	public function get_data() {		
        ...
        foreach ($rResult->result_array() as $aRow) {
			...
			$row[] = $aRow['tahun'];
            $row[] = $aRow['keterangan'];
			$row[] = '<a title="Edit Detail" href="logistik/forecast_pembelian/forecast_pembelian_detail/?hid='.$aRow['id'].'"><span class="icon-pencil5"></span></a>';
	...
	function create_header() {		
        $data = array(
			'no_transaksi'          => $this->m_forecast_pembelian->create_doc_no(date('n'), date('Y')),
		...
		$create_header = $this->m_forecast_pembelian->create_header($data);	
		$hid = $this->session->userdata('hid');
		if ($create_header) {
			redirect('logistik/forecast_pembelian/forecast_pembelian_detail/?hid='.$hid);
		} else {
			echo "<script>alert('Fail')</script>";
		}		 
	}
	
	function forecast_pembelian_detail() {
		...
		$data = array(
            ...
            'breadcrumb' => '<li>Logistik</li>
			                 <li>Manajemen Data</li>
			                 <li>Perencanaan Stok</li>
                             <li><a href="logistik/forecast_pembelian">Perencanaan Pembelian Produk</a></li>
			...
			'get_header' => $this->m_forecast_pembelian->get_header($hid),
			'get_produk_aktif' => $this->m_forecast_pembelian->get_produk_aktif(),
			'custom_scripts' => "<script type='text/javascript'>$('#menu_logistik_perencanaanpembelian').addClass('active');</script>"
		);
		$this->template->build('v_forecast_pembelian_detail', $data);		
	}
    
    function detail_list() {
		$hid = $this->input->get('hid');
        $data = $this->m_forecast_pembelian->detail_list($hid);
        if ($data->num_rows() > 0) {
            $i = 1;
	...
	function create_detail() {
		...
		$create_detail = $this->m_forecast_pembelian->create_detail($data);
        if ($create_detail) {
            echo "done";
        } else {
            echo "fail";
    ...
    function delete_item_detail() {        
        $del = $this->m_forecast_pembelian->delete_item_detail();        
        if ($del) {
            echo "done";
        } else {
            echo "fail";
    ...
    function update_item_detail() {		
        $del = $this->m_forecast_pembelian->update_item_detail();
        if ($del) {
            echo "done";
        } else {
            echo "fail";
	...
	function update_header() {
        $del = $this->m_forecast_pembelian->update_header();
        if ($del)
            echo "done";
        else
            echo "fail";
