...
class laporan_stok_gudang extends CI_Controller {
	
    public function __construct() {
		parent::__construct();
		$this->load->model('m_laporan_stok_gudang');
	...
	public function index() {
		$data = array(
            ...
            'breadcrumb' => '<li>Logistik</li>
							 <li>Laporan</li>
							 <li><a href="logistik/laporan_stok_gudang">Laporan Stok Gudang</a></li>',
			'page_icon' => 'icon-files',
			'page_title' => 'Laporan Stok Gudang',
			'page_subtitle' => 'Laporan Stok Gudang',
			'custom_scripts' => "<script type='text/javascript'>$('#menu_logistik_lapstokgudang').addClass('active')</script>"
		);
		$this->template->build('v_laporan_stok_gudang', $data);
	}
	
	public function ajax_list() {
        $list = $this->m_laporan_stok_gudang->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        foreach ($list as $stocks) {
            ...
            $row[] = $stocks->nama_perbekalan_produk;
			$row[] = $stocks->nama_kelompok_produk;
			$row[] = $stocks->nama_golongan_produk;
			$row[] = $stocks->nama_jenis_produk;
            $row[] = $stocks->stok;
            $row[] = $stocks->min_stok;
			$row[] = $stocks->max_stok;
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->m_laporan_stok_gudang->count_all(),
                        "recordsFiltered" => $this->m_laporan_stok_gudang->count_filtered(),
                        "data" => $data,
                );
	...
	function tampil_stok_produk() {		
        ...
        $data = $this->m_laporan_stok_gudang->get_stok_produk($id_produk);
	...
	function detail_stok($id_produk,
						 $batch_number,
						 $expired_date) {
        $data = $this->m_laporan_stok_gudang->detail_stok($id_produk,
												     $batch_number,
												     $expired_date);
        if ($data->num_rows() > 0) {
            $i = 1;
