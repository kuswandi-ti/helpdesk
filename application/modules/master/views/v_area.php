 <script>
  $(document).ready(function(){
    ...
    function update(id){
		$.ajax({
            url : '<?php echo base_url();?>index.php/area/area_view/' + id,
            type: "POST",
    ...
    $('.delete').button().click(function (){
        ...
        $.ajax({
            url : '<?php echo base_url();?>index.php/area/area_delete/' + id,
            type: "POST",
            success : function(result){
              var data=jQuery.parseJSON(result);
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form id="form_create" class="form-horizontal" action="area/area_create" method="POST">
                                <input type="hidden" name="created_by" value="<?php echo $data['user_name'];?>">
                                <div class="form-group">
                                    <label class="control-label col-md-3 col-sm-3 col-xs-12" for="nama">Nama Area <span class="required">*</span>
                                    </label>
                                ...
                                <div class="form-group">
                                  <label class="control-label col-md-3 col-sm-3 col-xs-12">Region Sales</label>
                                  <div class="col-md-6 col-sm-6 col-xs-12">
                                    <select class="form-control" name="region" id="region">
                                      <?php 
                                        /*for($i=0;$i<sizeof($data['area']);$i++)
                                        {
                                          echo "<option value=\"".$data['area'][$i]-> id_area."\">".$data['area'][$i]->nama."</option>";
                                        }*/
                                      ?>
                                    </select>
                                  </div>
            ...
            <div class="modal fade" id="update-form" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                              <form id="form_update" class="form-horizontal" action="area/area_update" method="POST">
                                <input type="hidden" name="created_by" value="<?php echo $data['user_name'];?>">
                                ...
                                <div class="form-group">
                                  <label class="control-label col-md-3 col-sm-3 col-xs-12">Area Sales</label>
                                  <div class="col-md-6 col-sm-6 col-xs-12">
                              ...
                              <?php 
                                if(sizeof($data['area']) != 0)
                                {
                                  ...
                                  { 
									...
									echo '<tr>';
                                    echo '<td>'.$no.'</td>';
                                    echo '<td>'.$data['area'][$i]->nama.'</td>';
                                    echo '<td class="text-center">';
                                    echo '<button value="'.$data['area'][$i]->id.'" id="btn_'.$i.'" class="update btn btn-labeled btn-success" title="Ubah"><i  class="glyphicon glyphicon-pencil"></i></button>';
                                    echo '<button value="'.$data['area'][$i]->id.'" id="btn_'.$i.'" class="del btn btn-labeled btn-danger" title="Hapus"><i class="glyphicon glyphicon-remove"></i></button>';
									echo '</td>';
                                    echo '</tr>';
                                  }
                                ...
                                {
                                  echo '<div class="alert alert-info text-center"><strong>Tidak terdapat area</strong></div>';
                                }
                              ?>
                  </div>
                </div>
