...
class Pengajuan_Pembayaran extends CI_Controller {
	
	var $view_pb_hdr		= 'ck_view_beli_pengajuanpembayaran_hdr';

	public function __construct() {
		parent::__construct();
		$this->load->model('M_Pengajuan_Pembayaran');
	...
	public function index() {
		$data = array(
			'title' => 'Pengajuan Pembayaran (PB)',
			'breadcrumb_home_active' => '',
            'breadcrumb' => '<li>Pembelian</li>
                             <li><a href="pembelian/pengajuan_pembayaran">Pengajuan Pembayaran (PB)</a></li>',
			'page_icon' => 'icon-receipt',
			'page_title' => 'Pengajuan Pembayaran (PB)',
			'page_subtitle' => 'Pengajuan pembayaran invoice pembelian dari supplier kepada bagian keuangan',
			'get_supplier' => $this->M_Pengajuan_Pembayaran->get_supplier(),
			'custom_scripts' => "<script type='text/javascript' src='assets/custom_script/beli_pengajuan_pembayaran.js'></script>"
		);
		$this->template->build('v_pengajuan_pembayaran', $data);
	}
	...
	public function get_data() {
        ...
        foreach ($rResult->result_array() as $aRow) {
			...
			$row[] = '<a title="Edit Detail" href="pembelian/pengajuan_pembayaran/pengajuan_pembayaran_detail/?hid='.$aRow['id_header'].'&sid='.$aRow['id_supplier'].'"><span class="icon-pencil4"></span></a>&nbsp;&nbsp;';
	...
	function create_header() {
		$data = array(
			'no_transaksi'          => $this->M_Pengajuan_Pembayaran->create_doc_no(date('n'), date('Y')),
		...
		$create_header = $this->M_Pengajuan_Pembayaran->header_create($data);
		$hid = $this->session->userdata('hid');
		$sid = $this->input->post('cbosupplier');
		if ($create_header) {
			redirect('pembelian/pengajuan_pembayaran/pengajuan_pembayaran_detail/?hid='.$hid.'&sid='.$sid);
		} else {
			echo "<script>alert('Fail')</script>";
		}
	}
	
	function pengajuan_pembayaran_detail() {
		...
		$data = array(
            ...
            'breadcrumb' => '<li>Pembelian</li>
                             <li><a href="pembelian/pengajuan_pembayaran">Pengajuan Pembayaran (PB)</a></li>
							 <li>Pengajuan Pembayaran (PB) Detail</li>',
			'page_icon' => 'icon-receipt',
			'page_title' => '',
			'page_subtitle' => '',			
			'custom_scripts' => "<script type='text/javascript' src='assets/custom_script/beli_pengajuan_pembayaran.js'></script>",
			'get_header' => $this->M_Pengajuan_Pembayaran->get_header($hid),
			'get_detail' => $this->M_Pengajuan_Pembayaran->get_detail($hid),
			'get_noinvoice' => $this->M_Pengajuan_Pembayaran->get_noinvoice($sid)
		);
		$this->template->build('v_pengajuan_pembayaran_detail', $data);		
	}
    ...
    function create_detail() {
        ...
        $create_detail = $this->M_Pengajuan_Pembayaran->detail_create($data);
        if ($create_detail) {
            $set_header = $this->M_Pengajuan_Pembayaran->get_sub_total_detail(
	...
	function detail_list() {
        $data = $this->M_Pengajuan_Pembayaran->detail_list($this->input->get('hid'));
        if ($data->num_rows() > 0) {
            $i = 1;
	...
	function delete_item_detail() {
        ...
        $del = $this->M_Pengajuan_Pembayaran->delete_item_detail($id_detail);
        if ($del) {
            $set_header = $this->M_Pengajuan_Pembayaran->get_sub_total_detail(
                                $this->input->post('id_header'),
                                $this->input->post('disc_persen_hdr'),
    ...
    function update_item_detail() {		
        $del = $this->M_Pengajuan_Pembayaran->update_item_detail();
        if ($del) {
            $set_header = $this->M_Pengajuan_Pembayaran->get_sub_total_detail(
                                $this->input->post('id_header'),
                                $this->input->post('disc_persen_hdr'),
	...
	function update_header() {		
        $del = $this->M_Pengajuan_Pembayaran->update_header();
        if ($del)
            echo "done";
        else
            echo "fail";
	...
	function get_info_invoice() {
		...
		$res = $this->M_Pengajuan_Pembayaran->get_info_invoice($no_invoice);
		if ($res->num_rows() > 0) {
			foreach ($res->result_array() as $row) {
				...
				$data['tgl_jatuh_tempo'] = date_format(new DateTime($row['tgl_jatuh_tempo']), $this->config->item('FORMAT_DATE_TO_DISPLAY'));
				$data['total_invoice'] = number_format($row['total_invoice']);
				$data['total_invoice_pengajuan'] = number_format($row['total_invoice_pengajuan']);
				$data['total_os_invoice'] = number_format($row['total_os_invoice']);
			}
		} else {
			$data['result'] = '';
			$data['tgl_invoice_supplier'] = date($this->config->item('FORMAT_DATE_TO_DISPLAY'));
			$data['tgl_jatuh_tempo'] = date($this->config->item('FORMAT_DATE_TO_DISPLAY'));
			$data['total_invoice'] = '0'; 
			$data['total_invoice_pengajuan'] = '0';
			$data['total_os_invoice'] = '0';
		}
