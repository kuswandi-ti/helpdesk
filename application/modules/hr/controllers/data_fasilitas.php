...
class data_fasilitas extends CI_Controller {
	...
	public function index() {
		$data = array(
            ...
            'breadcrumb' => '<li>Human Resource (HR)</li>
			                 <li>Master Data</li>
                             <li><a href="hr/data_fasilitas">Data Fasilitas</a></li>',
			'page_icon' => 'icon-files',
			'page_title' => 'Data Fasilitas',
			'page_subtitle' => 'Data Master Fasilitas',
			'custom_scripts' => "<script type='text/javascript'>$('#menu-hrga-master-data-fasilitas').addClass('active');</script>"
		);
		$this->template->build('v_data_fasilitas', $data);
	}
	...
	public function get_data() {
        ...
        // DB table to use
		$sTable = $this->tbl_data_fasilitas;
  	...
  	function data_fasilitas_view() {
		...
		$data_fasilitas = $this->m_data_fasilitas->data_fasilitas_view($id);
		foreach ($data_fasilitas->result_array() as $row) {
			$data['id'] = $row['id'];
			$data['nama'] = $row['nama'];
		}
	...
	function data_fasilitas_create() {
		$notifikasi = $this->m_data_fasilitas->data_fasilitas_create();
		if ($notifikasi == 1) {
	...
	function data_fasilitas_update() {
		$notifikasi = $this->m_data_fasilitas->data_fasilitas_update();
		if ($notifikasi == 1) {
			$notifikasi=" Selamat ! Anda berhasil";
		} else {
			$notifikasi=" Maaf ! Anda gagal";
	...
	function data_fasilitas_delete($id = '') {
		$notifikasi = $this->m_data_fasilitas->data_fasilitas_delete($id);
		if ($notifikasi == 1) {
			$notifikasi=" Selamat ! Anda berhasil";
		} else {
			$notifikasi=" Maaf ! Anda gagal";
