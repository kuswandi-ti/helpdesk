<?php
function split_kode_akun ($kode) {
	$kode = substr_replace($kode, '.', 6, 0);
	$kode = substr_replace($kode, '.', 4, 0);
	$kode = substr_replace($kode, '.', 2, 0);
	return $kode;
}
?>
 <script>
  $(document).ready(function(){
    ...
    function update(id){
		$.ajax({
            url : '<?php echo base_url();?>index.php/AsetKelompok/aset_kelompok_view/' + id,
            type: "POST",
            ...
            var data=jQuery.parseJSON(result)
                ...
                $("#kode_akun").val(data.akun_id).change();
            },
            error: function(){
              alert("failure");
            }
    ...
    $('.delete').button().click(function (){
        ...
        $.ajax({
            url : '<?php echo base_url();?>index.php/AsetKelompok/region_delete/' + id,
            type: "POST",
            success : function(result){
              var data=jQuery.parseJSON(result);
          ...
          <div class="">
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form id="form_create" class="form-horizontal" action="AsetKelompok/aset_kelompok_create" method="POST">
                                <input type="hidden" name="created_by" value="<?php echo $data['user_name'];?>">
                                <div class="form-group">
                                    <label class="control-label col-md-3 col-sm-3 col-xs-12" for="nama">Kelompok Aset <span class="required">*</span>
                                    </label>
                                  ...
                                  <div class="col-md-6 col-sm-6 col-xs-12">
									...
									<option></option>
                                      <?php 
                                        for($i=0;$i<sizeof($data['akun']);$i++)
                                        {
                                          echo "<option value=\"".$data['akun'][$i]->id."\">".$data['akun'][$i]->kode." ".$data['akun'][$i]->nama."</option>";
                                        }
                                      ?>
                                    </select>
                                  </div>
            ...
            <div class="modal fade" id="update-form" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                              <form id="form_update" class="form-horizontal" action="AsetKelompok/aset_kelompok_update" method="POST">
                                <input type="hidden" name="created_by" value="<?php echo $data['user_name'];?>">
			...
			<div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  ...
                  <div class="x_title">
                    <ul class="nav navbar-right panel_toolbox">
                      <a href="<?php echo base_url();?>index.php/Aset">Aset</a>
                    </ul>
                    <div class="clearfix"></div>
                  </div>
                  <div class="x_content">
                    <br />
                              <?php 
								...
								for($i=0;$i<sizeof($data['akun']);$i++)
                                {
                                  $arr_akun[$data['akun'][$i]->id] = $data['akun'][$i]->kode.' '.$data['akun'][$i]->nama;
                                }
								
								if(sizeof($data['aset_kelompok']) != 0)
                                {
                                  echo '
                                  ...
                                  { 
									...
									echo '<tr>';
                                    echo '<td>'.$no.'</td>';
                                    echo '<td>'.$data['aset_kelompok'][$i]->nama.'</td>';
                                    echo '<td>'.$data['aset_kelompok'][$i]->deskripsi.'</td>';
                                    echo '<td>'.split_kode_akun ($arr_akun[$data['aset_kelompok'][$i]->akun_id]).'</td>';
                                    echo '<td class="text-center">';
                                    echo '<button value="'.$data['aset_kelompok'][$i]->id.'" id="btn_'.$i.'" class="update btn btn-labeled btn-success" title="Ubah"><i  class="glyphicon glyphicon-pencil"></i></button>';
                                    echo '<button value="'.$data['aset_kelompok'][$i]->id.'" id="btn_'.$i.'" class="del btn btn-labeled btn-danger" title="Hapus"><i class="glyphicon glyphicon-remove"></i></button>';
                                    echo '</td>';
                                    echo '</tr>';
                                  }
                                ...
                                {
                                  echo '<div class="alert alert-info text-center"><strong>Tidak terdapat aset_kelompok</strong></div>';
                                }
                              ?>
                  </div>
                </div>
