$(document).on('click', '#showdetailnya',function(){
	...
	//alert(id_po);
	$.ajax({
		url:'penjualan/sales_order/loadDetail/'+id_po,
		type:'get',
	...
	$.ajax({
		...
		success:function(data){
			...
			$("#ppn").val(data[0]['ppn']);
			$("#ppnrp").val(parseInt(data[0]['ppn'])/100*(parseInt(data[0]['subtotal'])-parseInt(data[0]['diskon_rupiah'])));
...
$("#menu_so").addClass("active");
       $('#table_data').DataTable({
		"bProcessing": true,
		"bServerSide": true,
		"sServerMethod": "GET",
		"sAjaxSource": "penjualan/sales_order/populateSO/so",
		"iDisplayLength": 10,
