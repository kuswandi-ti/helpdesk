$('#menu_pembelian_pb').addClass('active');
...
$(document).ready(function() {
	...
	$('#table_data').DataTable({
		...
		"bServerSide": true,
		"sServerMethod": "GET",
		"sAjaxSource": "pembelian/pengajuan_pembayaran/get_data",
		"iDisplayLength": 10,
	...
	$("body").on("change", "#cbonoinvoice", function() {
		$.ajax({
			url: 'pembelian/pengajuan_pembayaran/get_info_invoice',
			data: {
			...
			success: function(data) {
				if (data.result == 'done') {
					...
					$('#txttgljatuhtempo').val(data.tgl_jatuh_tempo);
					$('#txttotalinvoice').val(data.total_os_invoice);
				} else {
					$('#txttglinvoice').val('');
					$('#txttgljatuhtempo').val('');
	...
	$("#submit_detail").click(function() {
		...
		var v_amount_invoice = $("#txttotalinvoice").asNumber({parseType: 'Float'});
        $.ajax({
            url: 'pembelian/pengajuan_pembayaran/create_detail',
			data: {
				id_header: v_id_header,
				no_invoice: v_no_invoice,
				amount_invoice: v_amount_invoice
			},
			dataType: 'json',
            type: 'post',
            success: function(data) {
    ...
    $("body").on("click", "#table_detail #hapus_detail", function() {
        ...
        if (del == true) {
            $.ajax({
                url: 'pembelian/pengajuan_pembayaran/delete_item_detail',
                type: 'post',
				dataType: 'json',
                data: {
					id_detail: id_detail,
    ...
    function detail_list() {
        ...
        $.ajax({
            url   : 'pembelian/pengajuan_pembayaran/detail_list/?hid='+hid,
            async : false,
            success : function(data) {
	...
	$("#simpan_header").click(function() {		
        ...
        $.ajax({
            url: 'pembelian/pengajuan_pembayaran/update_header',
			data: {
				id_header: v_id_header,
            ...
            success: function(data) {
                if (data == 'done')
					window.location = "pembelian/pengajuan_pembayaran";
            }
        });
    });
